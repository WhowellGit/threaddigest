...
misspelled ``THREADDIGEST_*`` variable makes it *fail*, at the documented exit 78, with the
...
def test_config_validate_refuses_an_unknown_variable_naming_it(
    ...
    (``THREADDIGEST_STATIC__BUDGET__PER_RUN_REQUESTZ``) was already refused by
    ``extra="forbid"``, naming the key rather than the variable; that half is pinned in
    ``tests/unit/test_settings.py``.
    """
    monkeypatch.setenv("THREADDIGEST_DATA_DIRR", str(isolated_data_dir))

    result = cli_runner.invoke(cli.app, ["config", "validate"])

    assert result.exit_code == 78, result.output
    assert "THREADDIGEST_DATA_DIRR" in result.output
...
def test_config_validate_never_echoes_a_secret_it_refused(
    ...
    """
    monkeypatch.setenv("THREADDIGEST_UI_PASSWORD", "open-sesame-1234")
    monkeypatch.setenv("THREADDIGEST_REDDIT_CLIENT_SECRET", "hunter2-secret-value")
    monkeypatch.setenv("THREADDIGEST_TYPO", "1")

    result = cli_runner.invoke(cli.app, ["config", "validate"])

    assert result.exit_code == 78, result.output
    assert "THREADDIGEST_TYPO" in result.output
