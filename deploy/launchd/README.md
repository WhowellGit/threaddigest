...
Two user agents run Thread Digest on this Mac while it is the production host
...
with `threaddigest`, `osascript`, `caffeinate` and `launchctl` replaced by recorders.
...
|---|---|---|
| 0 | ok | log only |
| 75 | another Thread Digest process holds the lock | log only |
...
A notification appears in Notification Center as a banner titled **Thread Digest** with the
status as subtitle and a body such as `Scheduled run failed (exit 1). Details:
http://127.0.0.1:8765/runs` (the `doctor` job points at `/system`). Set `THREADDIGEST_UI_URL`
...
trees also works. `threaddigest doctor` performs the same check from inside the application.
