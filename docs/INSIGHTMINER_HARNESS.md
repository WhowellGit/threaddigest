...
repository is an index of routing hooks and working habits, capped per file, snapshotted into
`memory-snapshot/` add-or-update only, and `make check` fails when the snapshot is behind live
memory or a project memory points at nothing. Sessions start in the repository root, because
...
rewritten document names unless its line says which milestone it waits on, counts
class-like names no code file defines into the same ceiling, and checks the live-facts
table against every listed mirror. Reference material
...
**Guards ledger rows** (`docs/runbook/GUARDS.md`): Active 37, External controls 5, Retired 0, Loosenings 3
...
- `.ratchets/docs.txt`: `dated_annotations`, `unresolved_class_names`
...
**Tools** (`tools/*.py`): `check_stamp.py`, `code_health.py`, `doc_policy.py`, `harness_page.py`, `hooks_status.py`, `make_demo_fixture.py`, `memory_snapshot.py`, `ratchet.py`, `render_plan.py`, `review_packet.py`, `vulture_whitelist.py`

**`make` targets**: `help`, `setup`, `hooks`, `check`, `memory-check`, `memory-export`, `code-health`, `doc-policy`, `lint-fix`, `ratchet-bump`, `ratchet-loosen`, `schema`, `test`, `fixture`, `run`, `plan-html`
