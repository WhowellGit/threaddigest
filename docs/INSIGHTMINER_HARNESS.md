...
by more than one or is stamped ahead of it, resolves every document path with a directory that a
rewritten document names, and checks the live-facts table against every listed mirror. Reference material
...
- `.ratchets/docs.txt`: `dated_annotations`
