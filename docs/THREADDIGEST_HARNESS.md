...
- **Nothing is advisory.** Every check either fails the build or lands in a ledger that needs an
  ...
  for approval and is recorded with its reason, and what ends the pause is a line Wes types at a
  terminal — the one step in this project an agent cannot take, because the command refuses a
  stdin that is not a terminal and an agent's shell has a pipe.
...
Four PreToolUse hooks in the project settings, in the path of every tool call: no bypass of the
...
a read-before-touch check that logs first and blocks only once its ledger shows it is right; and a
one-session-per-checkout lock, also log-first, that tells a second live session to work in its own
git worktree and releases the lock again when a session ends.
...
row, and the pause ends when Wes runs `make ratchet-approve KEY=…` at a terminal, which records
an approval for that one move, with its own expiry; a birth relaxation carries an expiry too. Gates in `tests/gates/` each ship a positive control.
...
- `tools/hooks/one_session_per_checkout.sh`: PreToolUse `Bash|Edit|Write|MultiEdit`; SessionEnd `*`; mode `log`
...
**Gate files** (`tests/gates/`, 28): `test_code_health`, `test_counters_deltas`, `test_data_dir_isolation`, `test_dependency_pins`, `test_doc_currency`, `test_doc_policy`, `test_harness_page`, `test_hooks`, `test_invariants_planted`, `test_known_issues_cite_collected_tests`, `test_layering`, `test_memory_snapshot`, `test_mutating_commands`, `test_no_bypass`, `test_no_imported_identifiers`, `test_no_sleep_under_pytest`, `test_no_tracked_daemon_state`, `test_no_write_outside_data_dir`, `test_pk_stability`, `test_pytest_config`, `test_ratchet`, `test_review_packet`, `test_review_register`, `test_routing_rows_resolve`, `test_rules_name_their_enforcer`, `test_sqlite3_confined`, `test_status_page`, `test_superseded_claims`

**Guards ledger rows** (`docs/runbook/GUARDS.md`): Active 40, External controls 5, Retired 0, Loosenings 5
...
**Tools** (`tools/*.py`): `check_stamp.py`, `code_health.py`, `doc_policy.py`, `harness_page.py`, `hash_remap.py`, `hooks_status.py`, `make_demo_fixture.py`, `memory_snapshot.py`, `private_terms.py`, `ratchet.py`, `render_plan.py`, `renumber_known_issue.py`, `review_packet.py`, `vulture_whitelist.py`

**`make` targets**: `help`, `setup`, `hooks`, `check`, `memory-check`, `memory-export`, `code-health`, `doc-policy`, `lint-fix`, `ratchet-bump`, `ratchet-loosen`, `ratchet-approve`, `schema`, `test`, `test-live`, `fixture`, `run`, `serve`, `plan-html`
