...
**Guards ledger rows** (`docs/runbook/GUARDS.md`): Active 41, External controls 5, Retired 0, Loosenings 4
...
- `.ratchets/tests.txt`: `<<<<<<< HEAD`, `asserts`, `collected`, `asserts`, `collected`, `>>>>>>> 362b53e (Bump the floors, relax the dead-code ceiling honestly, and record the round)`
...
**`make` targets**: `help`, `setup`, `hooks`, `check`, `memory-check`, `memory-export`, `code-health`, `doc-policy`, `lint-fix`, `ratchet-bump`, `ratchet-loosen`, `schema`, `test`, `test-live`, `fixture`, `run`, `serve`, `plan-html`
