...
| C-12 | Deletion markers are terminal from any prior state, and a scrub leaves no term bytes in the index or the file (FTS5 secure-delete, revision 0004) | `tests/unit/test_deletion.py::test_deleted_markers_are_terminal_from_any_prior`, `::test_scrub_states_exclude_holds_and_live`, `tests/db/test_fts.py::test_scrub_removes_the_entry`, `::test_scrub_with_secure_delete_leaves_no_term_bytes_without_an_optimize` | `core/deletion.py`, `db/fts.py` |
