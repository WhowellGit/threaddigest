...
from threaddigest.core.retry import MAX_RATE_LIMIT_WAIT_SECONDS
...
def check_settings_valid() -> Check:
    ...
    ``THREADDIGEST_*`` value must be reported by ``doctor``, and a ``Settings`` that some
...
def check_database_present(db_path: Path) -> Check:
    ...
    if not db_path.is_file():
        return Check(
            name=name,
            ok=False,
            detail=f"no database at {db_path}; run `threaddigest db init`",
            severity=CheckSeverity.ERROR,
        )
...
def check_alembic_at_head(engine: Engine) -> Check:
    ...
    if not at_head:
        detail = f"{detail}; run `threaddigest db upgrade`"
