...
2. Environment variables ``INSIGHTMINER_<FIELD>``. Nested static keys use ``__`` as the
   path separator, e.g. ``INSIGHTMINER_STATIC__BUDGET__PER_RUN_REQUESTS=500``.
3. ``config/settings.yaml`` (path overridable with ``INSIGHTMINER_SETTINGS_FILE``); this
...
before anything touches the filesystem, unless ``INSIGHTMINER_ALLOW_REAL_DATA_DIR`` is
set. The autouse fixture in ``tests/conftest.py`` points ``INSIGHTMINER_DATA_DIR`` at a
...
def default_data_dir() -> Path:
    """The data directory used when ``INSIGHTMINER_DATA_DIR`` is not set."""
    return _REPO_ROOT / "data"


def default_settings_file() -> Path:
    """The static settings file used when ``INSIGHTMINER_SETTINGS_FILE`` is not set."""
    ...
    def _refuse_real_data_dir_under_pytest(self) -> Settings:
        ...
        if self.data_dir == default_data_dir().resolve():
            msg = (
                ...
                "loaded. Set INSIGHTMINER_DATA_DIR to a temp directory (the autouse fixture "
