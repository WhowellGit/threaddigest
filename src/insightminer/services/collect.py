...
* **Every counter is final before :class:`~insightminer.services.invariants.InvariantContext`
...
:func:`~insightminer.services.sweep.sweep_all` and nothing else, so a Ctrl-C during the
...
from insightminer.services import invariants, runs, sweep
from insightminer.services.invariants import InvariantContext, Violation
from insightminer.services.runs import Counters, RunContext
from insightminer.services.sweep import SweepResult
...
def collect(
    ...
    *,
    ...
    budget :func:`~insightminer.services.runs.start_run` built from the settings.
    """
    if dry_run:
        return _collect_dry(
            engine,
